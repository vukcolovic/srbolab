...
func queryBuilderForCertificates(skip, take int, filter model.CertificateFilter, isCount bool) string {
	...
	if filter.TypeVehicle != "" {
		query = query + ` type_vehicle = ` + "'" + filter.TypeVehicle + "'" + ` AND `
	}
	if filter.Variant != "" {
		query = query + ` variant = ` + filter.Variant + ` AND `
	}
	if filter.VersionVehicle != "" {
		query = query + ` version_vehicle = ` + filter.VersionVehicle + ` AND `
	}
	if filter.EstimatedProductionYear != 0 {
		query = query + ` estimated_production_year = ` + strconv.Itoa(filter.EstimatedProductionYear) + ` AND `
	}
	if filter.EngineCode != "" {
		query = query + ` engine_code = ` + filter.EngineCode + ` AND `
	}
	if filter.EngineCapacity != 0 {
		query = query + ` engine_capacity = ` + strconv.Itoa(filter.EngineCapacity) + ` AND `
	}
	if filter.EnginePower != 0 {
		query = query + ` engine_power = ` + strconv.Itoa(filter.EnginePower) + ` AND `
	}
	if filter.Fuel != "" {
		query = query + ` fuel = ` + filter.Fuel + ` AND `
	}
